    ...
    for img in soup.findAll('img'):
        ...
        try:
            ...
            if area > 10000:
                ...
                img_guid = img["src"].split("images/")[1]
                path = "./temp_for_process/" + guid + "-" + img_guid
                output = open(path, "wb")
...
def process_image_ocr(image):
    ...
    if image.endswith(".gif"):
        print("skipping gif")
        return("skipped") # return dummy data for GIFs
    else:
        response = requests.get(image)
        img = Image.open(BytesIO(response.content))
        text = pytesseract.image_to_string(img)
        return(ocr_results.append(text + " "))
...
def get_images_data(images):
    ...
    for image in images:
        ...
        text_results = text_results + ocr_text + " "


        image_classification = classify_image.main_2("/Users/carl/sites/thinkful-data-science/coursework/final_capstone/data/images/6f5709220496529c352da1415c2f3d2becb35fb3-1.jpg")
        print(image_classification)
        image_results = "banana" + " "
